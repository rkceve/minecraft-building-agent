...
def build() -> Build:
    ...
    def dressed_window(frame, u, v, width, height, style="pointed", id=None):
        # Keep the reference reveal and tracery, using diorite stair sills
        # and wall-block rose outlines from the existing dressing family.
        b.palette = replace(pal, secondary="diorite")
        b.window(frame, u, v, width, height, style=style, id=id)
        b.palette = pal
        if style != "rose":
            # The DSL has no option for slender wall-block jambs or capitals.
            stem = max(1, height - ((width + 1) // 2) * 2)
            facing = {"n": "north", "s": "south", "e": "east", "w": "west"}[frame.face]
            for side in (-1, width):
                for row in range(stem):
                    block = "diorite_wall"
                    if row == stem - 1:
                        block = f"diorite_stairs[facing={facing},half=top,shape=straight,waterlogged=false]"
                    b.put(*frame.world(u + side, v + row, 1), block,
                          id=f"{id}_jamb_{side}_{row}")
    ...
    nave = {}
    # Raise the main roof four blocks, extending its supporting walls.
    nave["w"] = b.wall("w", 5, 11, 5, 39, 1, 16, bay=7, id="nave_west_side")
    nave["e"] = b.wall("e", 23, 11, 23, 39, 1, 16, bay=7, id="nave_east_side")
    nave["s"] = b.wall("s", 5, 39, 23, 39, 1, 16, bay=6, id="chancel_wall")
    ...
    for u in (3, 19):
        dressed_window(entrance, u, 2, 3, 7, id=f"entrance_lancet_{u}")
    dressed_window(gallery, 9, 2, 7, 7, style="rose", id="great_west_rose")
    for u in (3, 19):
        dressed_window(gallery, u, 2, 3, 6, id=f"facade_lancet_{u}")
    for frame in gallery_returns[:2]:
        u = 2 if frame.face == "w" else 3
        dressed_window(frame, u, 2, 3, 6, id=f"gallery_side_lancet_{frame.face}")
    for u in (3, 5, 19, 21):
        dressed_window(porch, u, 6, 1, 1, style="rose", id=f"porch_oculus_{u}")

    # Four tall bays and small high-level round lights on each nave side.
    # Two-block buttresses leave five-block (odd) wall panels. Centre each
    # window in that clear span, with identical world-z placement on both sides.
    pier_lines = (11, 18, 25, 32, 39)
    for face in ("w", "e"):
        frame = nave[face]
        for left, right in zip(pier_lines, pier_lines[1:]):
            z = (left + 2 + right - 1) // 2
            u = z - 11 - 1 if face == "w" else 39 - z - 1
            dressed_window(frame, u, 2, 3, 7, id=f"nave_{face}_lancet_{z}")
            for dz in (-1, 1):
                uc = z + dz - 11 if face == "w" else 39 - z - dz
                dressed_window(frame, uc, 10, 1, 1, style="rose", id=f"clerestory_{face}_{z}_{dz}")
        for z in pier_lines:
            u = z - 11 if face == "w" else 39 - z - 1
            b.buttress(frame, u, depth=1, height=15, id=f"nave_{face}_buttress_{z}")
        b.eave(frame, id=f"nave_{face}_cornice")
    dressed_window(nave["s"], 7, 3, 5, 7, id="east_chancel_window")
    ...
    for x0, name in ((3, "left"), (19, "right")):
        ...
        for x in (x0, x0 + 6):
            for z in (3, 9):
                b.corner_pillar(x, z, 0, 30, size=1, id=f"continuous_tower_pier_{x}_{z}")
    ...
    for u in (3, 10):
        dressed_window(annex["e"], u, 2, 3, 4, id=f"sacristy_window_{u}")
    ...
    dressed_window(apse["s"], 4, 2, 3, 5, id="rear_chancel_lancet")
    ...
    b.gable_roof(5, 10, 23, 39, 17, ridge="z", overhang=1, id="red_nave_roof")
    ...
    for x0, name in bell_stages:
        b.spire(x0, 3, x0 + 6, 9, 30, 14, id=f"dark_spire_{name}")
        cx = x0 + 3
        # Cross arms are the only custom spire geometry.
        b.box(cx - 1, 45, 6, cx + 1, 45, 6, "deepslate_tile_wall", id=f"cross_{name}")
        b.put(cx, 46, 6, "deepslate_tile_wall", id=f"cross_tip_{name}")
    b.palette = pal

    # Gallery floor, load-bearing interior posts, and braced timber ties.
    b.floor(3, 4, 25, 9, 9, id="organ_gallery_floor")
    for z in pier_lines[1:]:
        for x in (6, 22):
            b.corner_pillar(x, z, 0, 14, size=1, id=f"interior_post_{x}_{z}")
        b.beam(6, z, 22, z, 16, braces=True, id=f"nave_braced_tie_{z}")
