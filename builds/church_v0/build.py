...
def build() -> Build:
    ...
    b = Build(origin=(36, 1, 0), palette=pal, seed=1935)
    ...
    for face in ("w", "e"):
        frame = nave[face]
        # Center the tall openings vertically within the wall height.
        for z in (14, 21, 28, 35):
            u = z - 11 - 1 if face == "w" else 39 - z - 1
            v = (frame.height - 7 + 1) // 2
            b.window(frame, u, v, 3, 7, id=f"nave_{face}_lancet_{z}")
            for dz in (-1, 1):
    ...
    for x0, name in ((3, "left"), (19, "right")):
        ...
        for x in (x0, x0 + 6):
            for z in (3, 9):
                b.corner_pillar(x, z, 0, 28, size=1, id=f"continuous_tower_pier_{x}_{z}")
    ...
    for x0, name in bell_stages:
        b.spire(x0, 3, x0 + 6, 9, 28, 13, id=f"dark_spire_{name}")
        cx = x0 + 3
        # Cross arms are the only custom spire geometry.
        b.box(cx - 1, 42, 6, cx + 1, 42, 6, "deepslate_tile_wall", id=f"cross_{name}")
        b.put(cx, 43, 6, "deepslate_tile_wall", id=f"cross_tip_{name}")
    b.palette = pal
